...
## 2025-05-26 - TOTP Format String Optimization
**Learning:** `fmt.Sprintf` allocates memory and has extra overhead when formatting strings with padding zeros (e.g., `fmt.Sprintf("%06d", code)`).
**Action:** In performance-critical paths, zero-padding integers is significantly faster and allocation-free when using `strconv.FormatInt` combined with a pre-allocated zeros string slice (e.g., `"0000000000"[:padLen]`) rather than `fmt.Sprintf` with dynamic padding formats.
