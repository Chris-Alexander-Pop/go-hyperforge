...
## 2025-05-26 - TOTP String Formatting Optimization
**Learning:** `fmt.Sprintf` is computationally expensive for simple zero-padding in tight loops or high-frequency paths (like TOTP code generation), largely due to runtime format string parsing and reflection overhead.
**Action:** Replace `fmt.Sprintf("%%0%dd", digits)` dynamic zero-padding with `strconv.FormatInt` and a pre-allocated string of zeros (`"00000000000000000000"`) for padding. This approach is much faster and reduces heap allocations (from 9 allocs/op to ~2 allocs/op). Always declare the zero string as a `const` where possible, and provide a fallback like `strings.Repeat` for lengths exceeding the pre-allocated block to prevent logic regressions.
