...
## 2024-05-26 - Redis Cache Key Generation Optimization
**Learning:** Using `fmt.Sprintf` for constructing cache keys in high-throughput paths introduces unnecessary reflection and allocation overhead. In Redis adapters, string concatenation combined with `strconv` is significantly faster.
**Action:** Replace `fmt.Sprintf` with string concatenation (`"prefix:" + id`) and use `strconv` (e.g., `strconv.FormatUint`) for dynamic numbers to minimize allocations and improve performance in cache key generation.
