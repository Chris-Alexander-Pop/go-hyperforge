...
## 2024-06-06 - Replacing regex with manual parsing for hot path sanitization
**Learning:** Regex `MustCompile` combined with `ReplaceAllString` is convenient but heavily allocates memory and costs 10x more CPU time than a manual byte-by-byte scan.
**Action:** Use `strings.Builder` and manual iteration instead of regex replacements for heavily-called sanitization functions like `stripHTMLTags`.
