...
## 2024-06-26 - HTML Tag Stripping Performance
**Learning:** Regular expressions for simple string replacements like HTML tag stripping (`<[^>]*>`) carry significant overhead in Go. Manual string parsing using `strings.IndexByte` and `strings.Builder` can offer an order of magnitude performance improvement (e.g., from ~3000ns to ~200ns) by avoiding regexp compilation, state machine execution, and excess allocations.
**Action:** When performing simple bounding character replacements (like stripping between `<` and `>`), prefer manual linear string iteration using `strings.IndexByte` over regular expressions for hot paths.
