...
## 2025-05-26 - Redis Cache Key Generation Overhead
**Learning:** In high-throughput paths like Redis adapters (`pkg/auth/mfa/adapters/redis`, `pkg/auth/session/adapters/redis`), `fmt.Sprintf` introduces significant reflection and allocation overhead for cache key generation.
**Action:** Use string concatenation and `strconv` instead of `fmt.Sprintf` for constructing cache keys to minimize execution time and memory footprint (improves execution time by ~3x).
