...
## 2025-02-27 - Regex replacement for HTML Sanitization
**Learning:** Replaced `regexp.MustCompile("<[^>]*>")` with a manual zero-allocation (using `strings.Builder.Grow` and `strings.IndexByte`) parser in `stripHTMLTags`. Regex parsing in Go is often much slower than standard byte finding loops for simple operations.
**Action:** For simple string parsing and token skipping, especially in hot paths like sanitizers, prefer manual loop-based approaches with `strings.IndexByte` and `strings.Builder` over regular expressions.
