...
## 2025-05-26 - Parameter Placeholder Generation Optimization in Document Adapters
**Learning:** In Go, generating database query parameter placeholders or paths using `fmt.Sprintf` (e.g., `fmt.Sprintf(":v%d", i)`) introduces unnecessary CPU and allocation overhead due to reflection and parsing. Replacing this with `strconv.Itoa` and string concatenation (e.g., `":v" + strconv.Itoa(i)`) is significantly faster.
**Action:** Always prefer `strconv` and string concatenation over `fmt.Sprintf` when constructing simple strings dynamically in a loop or hot path.
