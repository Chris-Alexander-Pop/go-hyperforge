...
## 2025-06-29 - Redis Rate Limit Cache Key Optimization
**Learning:** Using `fmt.Sprintf` to construct cache keys and request IDs in the Redis distributed rate limiter creates unnecessary memory allocations and CPU overhead in a hot path.
**Action:** Replace `fmt.Sprintf` with direct string concatenation and `strconv.FormatInt` to reduce allocations from 24/40 bytes to 0/24 bytes per operation and improve speed.
