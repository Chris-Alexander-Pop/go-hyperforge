...
## 2025-02-23 - Avoid fmt.Sprintf for Simple String Concatenation in Redis Keys
**Learning:** Using `fmt.Sprintf` for constructing Redis keys (or any simple static text + variable string) incurs significant performance overhead due to memory allocations and reflection.
**Action:** Prefer direct string concatenation (`+`) combined with `strconv` for numbers to minimize reflection and allocation overhead, especially in high-throughput paths like cache adapters or session handling.
