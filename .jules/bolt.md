...
## 2023-10-24 - Optimize HTML tag stripping using manual parsing
**Learning:** In Go, replacing simple regexes with manual string parsing using `strings.IndexByte` and `strings.Builder` can lead to order-of-magnitude performance improvements (~10x for simple HTML tag stripping) and avoids ReDoS risks in hot paths.
**Action:** Default to manual string building for simple string manipulation instead of regexes in performance critical paths.
