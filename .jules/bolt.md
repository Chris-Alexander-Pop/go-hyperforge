...
## 2025-02-28 - Regex HTML Stripping Performance Bottleneck
**Learning:** Using `regexp.MustCompile` and `ReplaceAllString` for simple tasks like stripping HTML tags introduces significant overhead and memory allocations on hot paths like input sanitization.
**Action:** Replace simple regular expressions with manual string parsing in O(N) time using `strings.Builder` and `strings.IndexByte` to achieve up to 10x performance gains.
