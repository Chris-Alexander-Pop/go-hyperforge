...
## 2025-01-01 - Optimizing HTML Tag Stripping
**Learning:** In Go, replacing simple regular expressions (`regexp.MustCompile` and `ReplaceAllString`) with manual `strings.Builder` and `strings.IndexByte` iteration can provide substantial performance gains (up to 10x) and eliminate memory allocations on the hot path for HTML sanitization.
**Action:** Always prefer manual `O(N)` string scanning with `strings.IndexByte` over regex for simple parsing tasks on hot paths to minimize allocation and CPU overhead.
