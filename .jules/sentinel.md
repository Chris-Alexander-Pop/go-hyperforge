...
## 2025-05-05 - NoSQL Injection via Map Keys in Document Database Queries
**Vulnerability:** The CosmosDB adapter's `buildQuery` function directly concatenated map keys (`k`) into the query string (`sb.WriteString("c." + k)`) without validation. This allowed NoSQL injection if an attacker could control the map keys.
**Learning:** Parameterized queries only protect the values. Dynamic query construction must validate all structural inputs (like field names or map keys) against a strict allowlist.
**Prevention:** Enforce strict validation (e.g., using a regex like `^[a-zA-Z0-9_.]+$`) for any user-supplied identifier before concatenating it into a query.
