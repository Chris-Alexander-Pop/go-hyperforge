...
## 2025-07-08 - Fix symlink-based path traversal in ValidatePathInside
**Vulnerability:** The `ValidatePathInside` function was using basic string prefix matching (`strings.HasPrefix`) after `filepath.Join`, without resolving symlinks on the target path. An attacker could use a symlink inside the base directory pointing outside to bypass the check.
**Learning:** When validating paths to prevent traversal in Go, `filepath.EvalSymlinks` must be used to resolve the actual path. Furthermore, since `EvalSymlinks` fails if the target file does not exist yet, you must iteratively walk up the directory tree to resolve symlinks for existing parent directories before applying boundary checks.
**Prevention:** Always use `filepath.EvalSymlinks` on both the base directory and the target path when verifying if a path is inside a directory. Fail closed on broken symlinks by checking `os.Lstat` when `os.IsNotExist` is returned by `EvalSymlinks`.
