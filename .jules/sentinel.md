...
## 2024-03-31 - [Command Injection Validation Missing in Middleware]
**Vulnerability:** The global input sanitization middleware (`SanitizeMiddleware`) was missing validation for command injection via `validator.DetectCommandInjection()`, leaving inputs potentially vulnerable to downstream command execution.
**Learning:** System architecture features ML training adapters and Big Data compute clients that execute processes via `exec.CommandContext`. Because these adapters trust the globally sanitized inputs, the absence of `DetectCommandInjection` exposed the system to critical risk.
**Prevention:** Always verify that all available input validation methods are incorporated in global middleware, particularly when backend services rely on standard validation chains rather than re-validating inputs before executing processes.
