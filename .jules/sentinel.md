...
## 2024-04-12 - Missing Command Injection Validation in Middleware
**Vulnerability:** The global SanitizeMiddleware checked for SQL injection and Path Traversal, but failed to check for Command Injection, leaving downstream services (like ML training adapters and Big Data compute clients) vulnerable to `exec.CommandContext` injection via URL parameters.
**Learning:** Security middleware needs to validate against all execution contexts used in the backend architecture, not just web/database vectors. Overlooking command injection in global filters is critical when backend services execute system commands.
**Prevention:** Ensure the global middleware validator implementation encompasses all available detection vectors (SQLi, Path Traversal, Command Injection) offered by the security package.
