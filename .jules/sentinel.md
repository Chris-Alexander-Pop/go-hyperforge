...
## 2023-10-27 - Command Injection Detection in URL Queries
**Vulnerability:** Input validation middleware was checking for SQL Injection and Path Traversal, but omitted checks for Command Injection in URL query parameters.
**Learning:** Security validations (like `DetectCommandInjection`) need to be applied uniformly across entry points. The middleware acts as a gatekeeper and must cover all implemented validator types.
**Prevention:** When adding new validation logic (like command injection patterns), ensure it is integrated into the global sanitization/validation middleware (`SanitizeMiddleware`) rather than just relying on local usage.
