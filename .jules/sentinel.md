...
## 2025-04-02 - Missing Global Command Injection Validation
**Vulnerability:** Global middleware `SanitizeMiddleware` validated URL query parameters against SQL Injection and Path Traversal but lacked Command Injection checks, exposing ML training (`pkg/ai/ml/training`) and Big Data compute (`pkg/data/bigdata/compute`) clients that utilize `exec.CommandContext` to severe RCE risks.
**Learning:** Because underlying services directly execute shell commands via `exec.CommandContext`, global defense-in-depth at the API gateway layer is absolutely critical for this system's architecture.
**Prevention:** Ensure all global security middleware uniformly checks for SQLi, Path Traversal, and Command Injection before forwarding requests to backend compute clusters.
