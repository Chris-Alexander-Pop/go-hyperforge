...
## 2026-03-26 - Command Injection Missing in Global Validation
**Vulnerability:** The global API middleware `SanitizeMiddleware` validated URL query parameters for SQL Injection and Path Traversal, but failed to check for Command Injection.
**Learning:** Global middleware validates input but missed a critical attack vector (Command Injection), leaving the system potentially vulnerable to OS command injection even though underlying processes use exec.CommandContext.
**Prevention:** Ensure all relevant detection functions from the validator package are included in the global sanitization middleware.
