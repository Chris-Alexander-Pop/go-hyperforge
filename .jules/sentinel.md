...
## 2026-03-24 - Command Injection Bypass in Sanitize Middleware
**Vulnerability:** The `SanitizeMiddleware` failed to detect Command Injection in query parameters, only checking for SQL Injection and Path Traversal.
**Learning:** Global middleware input validation must comprehensively cover all expected injection types, especially when underlying systems (like ML/Big Data adapters) execute processes via `exec.CommandContext`.
**Prevention:** Ensure that all injection detection mechanisms (SQL, Path Traversal, Command Injection) are uniformly applied in security middlewares that act as a first line of defense.
