...
## 2025-03-09 - Cosmos DB NoSQL Injection
**Vulnerability:** Document database adapter directly concatenated unvalidated map keys into query strings and patch paths.
**Learning:** Map keys can be an injection vector when dynamically building queries or operations, even when parameterizing the values.
**Prevention:** Map keys must be strictly validated against an allowlist regex (e.g., ^[a-zA-Z0-9_.]+$) before being dynamically concatenated.
