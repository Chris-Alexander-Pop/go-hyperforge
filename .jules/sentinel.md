...
## 2024-05-24 - Timing Attack in MFA Recovery Code Validation
**Vulnerability:** The `Recover` methods in the Redis and Memory MFA adapters compared user-provided recovery codes (after hashing) against stored hashes using standard string equality (`==`). Standard string comparison exits early upon finding the first differing byte, creating a timing side-channel. This allows an attacker to potentially deduce the recovery code byte-by-byte by observing the time taken to process requests.
**Learning:** Security-sensitive string comparisons, such as checking passwords, tokens, hashes, or recovery codes, must never use standard equality operators (`==` or `!=`). Timing attacks exploit slight performance differences in these operators to guess secret data.
**Prevention:** Always use `crypto/subtle.ConstantTimeCompare([]byte(a), []byte(b))` when comparing cryptographic material, hashes, tokens, or any sensitive strings where timing leakage is a concern.
