...
## 2026-05-23 - NoSQL Injection in CosmosDB Query Construction
**Vulnerability:** The CosmosDB adapter constructed NoSQL queries and patch paths dynamically by blindly incorporating map keys without validation. This allowed NoSQL injection by manipulating map keys to alter query logic or structure.
**Learning:** Map keys provided in queries or updates are often derived from user input. In NoSQL databases, blindly using them to construct query paths or conditions without validation allows injection attacks.
**Prevention:** Map keys must be strictly validated against an allowlist regex (e.g., `^[a-zA-Z0-9_.]+$`) before being dynamically concatenated into query strings or patch operation paths to prevent NoSQL injection vulnerabilities.
