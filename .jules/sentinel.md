...
## 2025-04-02 - Missing Command Injection Validation in Middleware
**Vulnerability:** The global `SanitizeMiddleware` validated URL query parameters for SQL Injection and Path Traversal but failed to validate for Command Injection, leaving a critical gap in edge protection.
**Learning:** Security middleware must provide comprehensive coverage across all injection types, especially since underlying adapter logic might execute OS commands (e.g., `exec.CommandContext`).
**Prevention:** Ensure that global input validation layers incorporate detection for all dangerous characters and patterns (SQLi, Path Traversal, and Command Injection) uniformly before routing requests.
