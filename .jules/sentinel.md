...
## 2024-05-25 - NoSQL Injection in CosmosDB Adapter
**Vulnerability:** Unvalidated map keys were being dynamically concatenated into query strings and patch operation paths in the CosmosDB adapter.
**Learning:** Even when using parameterized values for query arguments, NoSQL databases can be vulnerable to injection if the keys (field names) used in the queries or updates are user-controlled and unvalidated.
**Prevention:** Always strictly validate user-supplied keys against an allowlist regex (e.g., `^[a-zA-Z0-9_.]+$`) before using them to construct database operations or queries.
