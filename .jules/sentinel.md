...
## 2026-03-30 - [Command Injection Defense]
**Vulnerability:** Missing command injection detection in request sanitization middleware.
**Learning:** The system architecture includes ML training adapters (`pkg/ai/ml/training`) and Big Data compute clients (`pkg/data/bigdata/compute`) that execute underlying processes via `exec.CommandContext`, making global middleware input validation against command injection critical.
**Prevention:** Ensure `validator.DetectCommandInjection` is consistently applied in any middleware responsible for input sanitization.
