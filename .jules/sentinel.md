...
## 2026-02-06 - NoSQL Injection via Unvalidated Map Keys
**Vulnerability:** The CosmosDB adapter dynamically constructed SQL queries (`buildQuery`) and Patch operations by blindly concatenating map keys provided in the `query` or `update` arguments. This allowed an attacker to inject arbitrary SQL or manipulate patch paths by passing malformed keys (e.g., `id = @p0 OR 1=1 --`).
**Learning:** Even when using parameterized queries for values (like `@p0`), dynamically concatenating keys or field names into a query string remains a critical NoSQL injection vector.
**Prevention:** In document database adapters, map keys must be strictly validated against an allowlist regex (e.g., `^[a-zA-Z0-9_.]+$`) before being dynamically concatenated into query strings or patch operation paths.
