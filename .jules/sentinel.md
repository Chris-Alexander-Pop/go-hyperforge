...
## 2025-04-04 - Missing Command Injection Protection in Global Middleware
**Vulnerability:** Global HTTP middleware `SanitizeMiddleware` checked for SQL injection and Path Traversal but completely omitted checking for Command Injection, leaving downstream services exposed to shell injection payloads via query parameters.
**Learning:** This architectural flaw is especially critical in this codebase because ML training adapters (`pkg/ai/ml/training/adapters/tensorflow` and `pytorch`) and Big Data clients (`pkg/data/bigdata/compute/spark`) execute system processes via `exec.CommandContext` using user-provided paths and arguments.
**Prevention:** Always integrate all available input validation heuristics (`DetectCommandInjection` along with `DetectSQLInjection` and `DetectPathTraversal`) at the global middleware boundary to ensure defense-in-depth for downstream processes that interact with the OS.
