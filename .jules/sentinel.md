...
## 2026-03-22 - [Add Command Injection Protection to SanitizeMiddleware]
**Vulnerability:** URL query parameters and referer headers weren't explicitly checked for command injection patterns during sanitization, although path traversal and SQL injection were checked.
**Learning:** In a full system design context where request inputs could be forwarded or parsed by backend tools like ML trainers or Big Data processors (e.g. `exec.Command` in `pkg/ai/ml/training/adapters/tensorflow` or `pkg/data/bigdata/compute/spark`), malicious queries containing shell metacharacters could trigger command injection.
**Prevention:** Always validate against command injection across all input sanitizers globally (like in `pkg/api/middleware/request_security.go`) before passing data down to subcomponents that may run sub-processes.
