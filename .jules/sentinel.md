...
## 2026-04-25 - Prevent NoSQL Injection in CosmosDB Query Builder
**Vulnerability:** The CosmosDB document adapter dynamically constructs string queries (`buildQuery` function in `pkg/database/document/adapters/cosmosdb/cosmosdb.go`) using map keys provided in the `query` argument without validation. This allows an attacker to inject arbitrary text, bypassing parameters and compromising database integrity (NoSQL injection).
**Learning:** Document database query builder logic must validate all map keys used to construct query texts to prevent NoSQL injection.
**Prevention:** Always validate map keys against an allowlist pattern (e.g., `^[a-zA-Z0-9_.]+$`) before embedding them in dynamically built query strings.
