...
## 2025-04-05 - Timing Attack Vulnerability in MFA Recovery
**Vulnerability:** Recovery code verification in the memory and redis MFA adapters used standard string equality (`==`) when comparing hashed input codes against stored hashes.
**Learning:** In MFA authentication adapters, sensitive tokens like hashed recovery codes must be compared securely using `crypto/subtle.ConstantTimeCompare` rather than standard string equality (`==`) to prevent timing attacks.
**Prevention:** Always use `subtle.ConstantTimeCompare` for comparing hashes or tokens representing sensitive secrets.
