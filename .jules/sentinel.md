...
## 2025-05-18 - Fix Symlink-Based Path Traversal Bypass
**Vulnerability:** The path traversal validation using `filepath.Abs` and `filepath.Join` followed by string prefix checking did not adequately resolve symlinks within the target path, allowing a malicious user to craft a symlink that points outside the allowed base directory and access arbitrary files.
**Learning:** Checking path prefixes without evaluating symlinks is inherently insecure, as symlinks can dynamically alter the actual target location on the filesystem to a location outside the expected boundaries. When validating paths to prevent traversal in Go, if `filepath.EvalSymlinks` returns `os.ErrNotExist` for a path component that is known to exist, it indicates a broken symlink and validation logic must fail closed and return an error to prevent path traversal bypasses.
**Prevention:** Always evaluate symlinks fully (e.g., iteratively walk up the directory tree using `filepath.Dir` to resolve symlinks for existing parent directories) before applying string-based boundary checks like `strings.HasPrefix`.
