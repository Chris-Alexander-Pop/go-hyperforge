...
## 2025-04-09 - [CRITICAL] Missing Command Injection Validation in Middleware
**Vulnerability:** Global `SanitizeMiddleware` was checking for SQL Injection and Path Traversal, but completely missed Command Injection validation.
**Learning:** Because ML training adapters (`pkg/ai/ml/training`) and Big Data compute clients (`pkg/data/bigdata/compute`) use `exec.CommandContext` extensively, command injection is a first-class threat vector in this architecture.
**Prevention:** Ensure that all centralized input validation middlewares natively include `DetectCommandInjection` checks alongside other injection checks, given the widespread use of underlying OS process execution.
