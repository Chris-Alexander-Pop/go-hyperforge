...
## 2024-06-03 - CosmosDB NoSQL Injection
**Vulnerability:** Document database adapter keys were unvalidated, allowing potential NoSQL injection vectors.
**Learning:** Map keys used for query/patch construction in NoSQL adapters must be strictly validated since they bypass parameterized values.
**Prevention:** Use a regex like `^[a-zA-Z0-9_.]+$` to strictly validate all field names and map keys before concatenating them into adapter queries or path structures.
