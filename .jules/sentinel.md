...
## 2026-02-06 - NoSQL Injection in CosmosDB Query Builder
**Vulnerability:** The CosmosDB adapter's `buildQuery` function dynamically concatenated map keys directly into the SQL query string (`c.<key> = @pX`) without validation. This allowed attackers to construct NoSQL injection payloads if they controlled the keys of the query map (e.g., `id = '1' OR 1=1`).
**Learning:** While values were properly parameterized using the Azure SDK (`@pX`), dynamic keys are inherently difficult to parameterize in SQL-like NoSQL query languages. Blindly trusting map keys during query construction circumvents parameterization benefits.
**Prevention:** Always apply strict allowlist validation (e.g., `regexp.MustCompile("^[a-zA-Z0-9_.]+$")`) to dynamic query keys before concatenating them into query strings in NoSQL and document databases.
