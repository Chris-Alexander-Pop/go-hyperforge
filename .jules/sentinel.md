...
## 2025-02-17 - [HIGH] Add command injection detection to sanitize middleware
**Vulnerability:** The `SanitizeMiddleware` checked for SQL injection and Path Traversal but missed Command Injection patterns.
**Learning:** The `SanitizeMiddleware` iteratively decoded and validated query parameters and headers, but missed crucial defense-in-depth checks against command injection via dangerous characters like `;`, `|`, `&`, `$`, `` ` ``.
**Prevention:** Add `validator.DetectCommandInjection` checks in the request sanitization middleware alongside `validator.DetectSQLInjection` and `validator.DetectPathTraversal` to reject payloads attempting command execution on the host.
