...
## 2025-07-06 - Prevent Path Traversal via Symlink Resolution
**Vulnerability:** In Go, string-based path traversal checks like `strings.HasPrefix` on paths combined with `filepath.Join` are vulnerable to bypasses if the target path contains symlinks pointing outside the intended base directory.
**Learning:** `filepath.Join` does not resolve symlinks. Before checking boundaries using prefixes, paths must be resolved to their real absolute paths using `filepath.EvalSymlinks`. For paths where the target does not yet exist, symlink evaluation must iteratively walk up the directory tree to properly evaluate all existing parent directories.
**Prevention:** Always use `filepath.EvalSymlinks` on both the `baseDir` and the full `targetPath` (walking up the parent chain if necessary to resolve partial paths) before applying boundary checks like `strings.HasPrefix`.
