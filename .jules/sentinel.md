...
## 2026-02-05 - NoSQL Injection in CosmosDB Query Building
**Vulnerability:** The CosmosDB adapter constructed queries by dynamically concatenating map keys into the query string (`sb.WriteString(k)`). Since keys were user-supplied and not validated, an attacker could supply a crafted key (e.g., `age = 30 OR 1=1 --`) to manipulate the query structure, leading to NoSQL injection.
**Learning:** Document database queries often rely on dynamic key structures. Unlike values, which can often be parameterized, keys are often interpolated directly into the query string, making them highly susceptible to injection if unvalidated.
**Prevention:** Strictly validate any user-supplied map keys against an allowlist regular expression (e.g., `^[a-zA-Z0-9_.]+$`) before interpolating them into dynamic query strings, ensuring they conform to expected field name formats.
