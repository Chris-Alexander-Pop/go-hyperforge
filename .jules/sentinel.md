...
## 2026-02-05 - NoSQL Injection via Map Key Concatenation
**Vulnerability:** The CosmosDB document adapter constructed query parameters and update operations by blindly concatenating map keys (e.g., `fmt.Sprintf("c.%s = @p%d", k, i)` and `fmt.Sprintf("/%s", k)`) without any validation. An attacker controlling the query keys could inject arbitrary conditions or paths (e.g., `id OR 1=1`), leading to NoSQL injection.
**Learning:** While query values are typically parameterized natively by SDKs (like `@p0`), map keys are often implicitly trusted and dynamically concatenated by database adapters constructing queries. This is a common NoSQL injection vector.
**Prevention:** Always validate map keys against a strict allowlist regex (e.g., `^[a-zA-Z0-9_.]+$`) before dynamically concatenating them into database queries or update paths.
