...
## 2025-02-20 - Fix Path Traversal via Symlinks in Validation
**Vulnerability:** `ValidatePathInside` relied only on string matching (`filepath.Join` and `strings.HasPrefix`) which fails to detect path traversal when a user-controlled path contains symlinks pointing outside the base directory.
**Learning:** `filepath.EvalSymlinks` is required to securely validate paths, but it returns `os.ErrNotExist` for paths that don't exist yet or for broken symlinks. To validate paths for files that will be created, we must walk up the directory tree to resolve symlinks of the closest existing parent directory. If an existing path component fails symlink evaluation, it's a broken symlink and must fail closed.
**Prevention:** Always use `filepath.EvalSymlinks` to resolve actual paths before prefix matching. Iteratively check parent directories for non-existent targets, and explicitly check for broken symlinks using `os.Lstat` when `EvalSymlinks` fails.
