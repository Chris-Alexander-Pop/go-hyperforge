...
## 2024-04-08 - Missing Global Command Injection Validation
**Vulnerability:** Global request sanitization middleware (`SanitizeMiddleware`) was missing `validator.DetectCommandInjection` checks, exposing downstream ML and Big Data components that use `exec.CommandContext` to Remote Code Execution (RCE) via query parameters.
**Learning:** Defense in depth requires that global input validation middleware uniformly applies all available validation rules (SQLi, Path Traversal, and Command Injection). Partial implementation creates blind spots for specific attack vectors.
**Prevention:** Always audit global sanitization middleware to ensure it leverages all available security detection functions in the validator package, especially when the architecture involves direct shell execution.
