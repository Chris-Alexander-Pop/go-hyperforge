...
## 2024-05-29 - CosmosDB Adapter NoSQL Injection
**Vulnerability:** Document database adapter map keys concatenated dynamically into `PatchOperations` paths and SQL `WHERE` clauses without validation.
**Learning:** Must validate keys against a safe allowlist (`^[a-zA-Z0-9_.]+$`) before building queries/paths.
**Prevention:** Use pre-compiled Regex to allowlist field names to prevent malicious manipulation of query structures.
