...
## 2026-04-17 - NoSQL Injection in CosmosDB Query Builder
**Vulnerability:** The `buildQuery` function in `pkg/database/document/adapters/cosmosdb/cosmosdb.go` was vulnerable to NoSQL injection because user-supplied keys from the `query` map were directly concatenated into the CosmosDB query string (`"c." + k + " = " + paramName`). This allowed attackers to inject arbitrary SQL statements or logical operators (e.g. `name = 'a' OR 1=1; --`) to bypass access controls or exfiltrate data.
**Learning:** Even when parameterized queries are used for the values, failing to validate or parameterize the keys/field names themselves can still result in a critical injection vulnerability in document database wrappers that dynamically build queries.
**Prevention:** Always validate user-provided structural components of queries (like keys or field names) against a strict allowlist or a restrictive regex (e.g., `^[a-zA-Z0-9_.]+$`) before string concatenation, as parameterization is often unsupported for schema identifiers.
