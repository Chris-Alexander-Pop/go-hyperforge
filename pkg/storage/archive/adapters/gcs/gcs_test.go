...
import (
	...
	"github.com/chris-alexander-pop/go-hyperforge/pkg/storage/archive/adapters/gcs"
	"github.com/stretchr/testify/require"
)

func TestGCSArchiveAdapter(t *testing.T) {
	api := gcs.NewMemoryObjectAPI()
	store, err := gcs.NewFromAPI(api, gcs.Config{
		Bucket:             "cold",
...
func TestGCSNewRequiresBucket(t *testing.T) {
	_, err := gcs.NewFromAPI(gcs.NewMemoryObjectAPI(), gcs.Config{})
	require.Error(t, err)
}
