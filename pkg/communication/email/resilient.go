...
// Retry settings are typically wired from Config.RetryMax / Config.RetryBackoff.
...
// ResilientConfig configures the resilient email sender wrapper.
type ResilientConfig struct {
	CircuitBreakerEnabled   bool          `env:"EMAIL_CB_ENABLED" env-default:"false"`
	CircuitBreakerThreshold int64         `env:"EMAIL_CB_THRESHOLD" env-default:"5"`
	CircuitBreakerTimeout   time.Duration `env:"EMAIL_CB_TIMEOUT" env-default:"30s"`

	RetryEnabled     bool          `env:"EMAIL_RETRY_ENABLED" env-default:"true"`
	RetryMaxAttempts int           `env:"EMAIL_RETRY_MAX" env-default:"3"`
...
// When cfg comes from email.Config, use ResilientConfigFrom.
...
// ResilientConfigFrom maps email.Config retry fields into a ResilientConfig.
...
// Send dispatches a single email with retry/circuit-breaker protection.
...
// SendBatch dispatches multiple emails with retry/circuit-breaker protection.
