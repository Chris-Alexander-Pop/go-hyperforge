...
// (auto-escaping for HTML contexts).
type Engine struct {
	templates *htmltemplate.Template
	mu        *concurrency.SmartRWMutex
}

// New creates an html/template engine. When cfg.Dir is set, all *.html, *.htm,
// and *.tmpl files in that directory are parsed.
func New(cfg tmpl.Config) (*Engine, error) {
	e := &Engine{
		templates: htmltemplate.New(""),
