...
var keyAllowlistRegex = regexp.MustCompile(`^[a-zA-Z0-9_.]+$`)
...
func (a *Adapter) Update(ctx context.Context, collection string, filter map[string]interface{}, update map[string]interface{}) error {
	...
	for k, v := range update {
		if !keyAllowlistRegex.MatchString(k) {
			return errors.InvalidArgument(fmt.Sprintf("invalid update key: %s", k), nil)
		}
...
func buildQuery(query map[string]interface{}) (string, []azcosmos.QueryParameter, error) {
	...
	for k, v := range query {
		if !keyAllowlistRegex.MatchString(k) {
			return "", nil, errors.InvalidArgument(fmt.Sprintf("invalid query key: %s", k), nil)
		}
		if i > 0 {
			sb.WriteString(" AND ")
		}
