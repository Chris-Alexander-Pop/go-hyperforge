...
func (a *Adapter) Update(ctx context.Context, collection string, filter map[string]interface{}, update map[string]interface{}) error {
	...
	for k, v := range update {
		if !validKeyRegex.MatchString(k) {
			return errors.InvalidArgument("invalid characters in update key", nil)
		}
...
func buildQuery(query map[string]interface{}) (string, []azcosmos.QueryParameter, error) {
	// Validate all keys first to prevent injection
	for k := range query {
		if !validKeyRegex.MatchString(k) {
			return "", nil, errors.InvalidArgument("invalid characters in query key", nil)
		}
	}
