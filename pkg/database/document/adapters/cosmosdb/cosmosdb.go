...
var queryKeyRegex = regexp.MustCompile(`^[a-zA-Z0-9_.]+$`)
...
func buildQuery(query map[string]interface{}) (string, []azcosmos.QueryParameter, error) {
	...
	i := 0
	for k, v := range query {
		if !queryKeyRegex.MatchString(k) {
			return "", nil, errors.InvalidArgument(fmt.Sprintf("invalid query key format: %s", k), nil)
		}
