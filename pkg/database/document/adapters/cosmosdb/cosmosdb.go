...
func (a *Adapter) Update(ctx context.Context, collection string, filter map[string]interface{}, update map[string]interface{}) error {
	...
	for k, v := range update {
		if !keyRegex.MatchString(k) {
			return errors.InvalidArgument("invalid update key: possible injection detected", nil)
		}
...
func buildQuery(query map[string]interface{}) (string, []azcosmos.QueryParameter, error) {
	...
	for k, v := range query {
		if !keyRegex.MatchString(k) {
			return "", nil, errors.InvalidArgument("invalid query key: possible injection detected", nil)
		}
		if i > 0 {
			sb.WriteString(" AND ")
		}
