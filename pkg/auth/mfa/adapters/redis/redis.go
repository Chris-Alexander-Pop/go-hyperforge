...
func (p *MFAProvider) key(userID string) string {
	// ⚡ Bolt: Use string concatenation instead of fmt.Sprintf to reduce allocations
	return "auth:mfa:" + userID
}

func (p *MFAProvider) usedKey(userID string, counter uint64) string {
	// ⚡ Bolt: Use strconv and concatenation to optimize key generation in critical path
