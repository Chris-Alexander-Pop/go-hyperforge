...
func (t *TOTP) generateCodeForCounter(secret string, counter uint64) (string, error) {
	...
	if len(codeStr) < t.config.Digits {
		const zeros = "00000000000000000000" // 20 zeros
		paddingLen := t.config.Digits - len(codeStr)
		if paddingLen <= len(zeros) {
			return zeros[:paddingLen] + codeStr, nil
		}
		return strings.Repeat("0", paddingLen) + codeStr, nil
	}
	return codeStr, nil
}
