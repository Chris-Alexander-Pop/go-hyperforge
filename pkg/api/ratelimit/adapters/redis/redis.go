...
func (l *DistributedLimiter) fixedWindowAllow(ctx context.Context, key string, limit int64, period time.Duration) (*ratelimit.Result, error) {
	// Optimization: Use string concatenation instead of fmt.Sprintf to avoid reflection overhead and memory allocations in the hot path.
...
func (l *DistributedLimiter) slidingWindowAllow(ctx context.Context, key string, limit int64, period time.Duration) (*ratelimit.Result, error) {
	// Optimization: Use string concatenation and strconv instead of fmt.Sprintf to avoid reflection overhead and memory allocations in the hot path.
