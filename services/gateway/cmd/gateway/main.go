...
import (
	...
	"github.com/chris-alexander-pop/go-hyperforge/services/gateway/server"
...
func main() {
	...
	srv, err := server.New(cfg, tokens)
	if err != nil {
		logger.L().Error("gateway init failed", "error", err)
		os.Exit(1)
	}

	logger.L().Info("gateway service starting", "port", cfg.Port, "service", cfg.ServiceName)

	go func() {
		if err := srv.Start(); err != nil {
			logger.L().Error("gateway server stopped", "error", err)
		}
	}()
