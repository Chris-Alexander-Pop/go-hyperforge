...
import (
	...
	"github.com/chris-alexander-pop/go-hyperforge/services/user/server"
)
...
func main() {
	...
	srv := server.New(cfg)
	logger.L().Info("user service starting", "port", cfg.Port, "service", cfg.ServiceName)

	go func() {
		if err := srv.Start(); err != nil {
			logger.L().Error("user server stopped", "error", err)
		}
	}()
