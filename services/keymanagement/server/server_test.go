...
import (
	...
	"github.com/chris-alexander-pop/go-hyperforge/services/keymanagement/server"
)
...
func TestHealthEncryptDecrypt(t *testing.T) {
	...
	encBody, _ := json.Marshal(map[string]string{
		"key_id":    "default",
		"plaintext": "secret-value",
	})
	encResp, err := http.Post(ts.URL+"/v1/keys/encrypt", "application/json", bytes.NewReader(encBody))
	if err != nil {
	...
	decBody, _ := json.Marshal(map[string]string{
		"key_id":            "default",
		"ciphertext_base64": encOut.CiphertextBase64,
	})
	decResp, err := http.Post(ts.URL+"/v1/keys/decrypt", "application/json", bytes.NewReader(decBody))
	if err != nil {
...
func TestEncryptMissingKeyID(t *testing.T) {
	srv := server.New(server.Config{Port: "0"})
	ts := httptest.NewServer(srv.Echo())
	t.Cleanup(ts.Close)

	body, _ := json.Marshal(map[string]string{"plaintext": "x"})
	resp, err := http.Post(ts.URL+"/v1/keys/encrypt", "application/json", bytes.NewReader(body))
	if err != nil {
		t.Fatalf("encrypt: %v", err)
	}
