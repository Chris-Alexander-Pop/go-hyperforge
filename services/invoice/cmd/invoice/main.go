...
import (
	...
	"github.com/chris-alexander-pop/go-hyperforge/services/invoice/server"
...
func main() {
	...
	srv := server.New(cfg)
	logger.L().Info("invoice service starting", "port", cfg.Port, "service", cfg.ServiceName)

	go func() {
		if err := srv.Start(); err != nil {
			logger.L().Error("invoice server stopped", "error", err)
		}
	}()
